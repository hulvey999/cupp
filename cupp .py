...
def learn_password_patterns(password_list):
    """Learn patterns from existing passwords using unsupervised learning"""
    try:
        if not password_list:
            return None, None
        
        # Convert passwords to character sequences
        char_sequences = [' '.join(list(pwd)) for pwd in password_list]
        
        # Create TF-IDF features
        vectorizer = TfidfVectorizer(analyzer='char', ngram_range=(2, 3))
        X = vectorizer.fit_transform(char_sequences)
        
        # Perform clustering
        n_clusters = min(5, len(password_list))  # Use at most 5 clusters
        kmeans = KMeans(n_clusters=n_clusters, random_state=42)
        clusters = kmeans.fit_predict(X)
        
        return vectorizer, kmeans
    except Exception as e:
        print(f"Error in learning password patterns: {e}")
        return None, None


def generate_passwords_from_patterns(vectorizer, kmeans, existing_passwords, n_passwords=10):
    """Generate new passwords based on learned patterns"""
    try:
        if vectorizer is None or kmeans is None or not existing_passwords:
            return []
        
        # Get cluster centers
        centers = kmeans.cluster_centers_
        
        # Generate passwords from each cluster
        generated_passwords = []
        for center in centers:
            # Find closest existing password to center
            closest_idx = np.argmin(np.linalg.norm(vectorizer.transform([' '.join(list(pwd))]) - center, axis=1))
            base_password = list(existing_passwords[closest_idx])
            
            # Generate variations
            for _ in range(n_passwords // len(centers)):
                # Randomly modify the base password
                new_password = base_password.copy()
                # Randomly change some characters
                for i in range(len(new_password)):
                    if np.random.random() < 0.3:  # 30% chance to change each character
                        new_password[i] = chr(np.random.randint(33, 127))
                generated_passwords.append(''.join(new_password))
        
        return generated_passwords
    except Exception as e:
        print(f"Error in generating passwords from patterns: {e}")
        return []
...
def improve_dictionary(file_to_open):
    ...
    try:
        ...
        # Add machine learning based password generation
        if os.path.isfile(file_to_open):
            with open(file_to_open, 'r') as f:
                existing_passwords = [line.strip() for line in f.readlines()]
            
            # Learn patterns from existing passwords
            vectorizer, kmeans = learn_password_patterns(existing_passwords)
            
            if vectorizer and kmeans:
                # Generate new passwords based on patterns
                ml_passwords = generate_passwords_from_patterns(vectorizer, kmeans, existing_passwords)
                unique_list_finished.extend(ml_passwords)

        print_to_file(file_to_open + ".cupp.txt", unique_list_finished)
        fajl.close()

    except Exception as e:
        print(f"Error in improve_dictionary: {e}")
        if 'fajl' in locals():
            fajl.close()
